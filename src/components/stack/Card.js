...
const { height, width } = configs.screen;
...
class CardStack extends Component {
    ...
    initDeck() {
        ...
        this.setState({ cards,  cardA, cardB, sindex: initialIndexB + 1 });
    }
    ...
    _goBack(direction) {
        ...
        let update = {};
        if (topCard === 'cardA') {
            update = { ...update, cardB: cards[previusCardIndex] }
        } else {
            update = { ...update, cardA: cards[previusCardIndex] }
        }

        this.setState({ ...update, topCard: (topCard === 'cardA') ? 'cardB' : 'cardA', sindex: sindex - 1  }, () => {
            switch (direction) {
                case 'top':
    ...
    _nextCard(direction, x, y, duration = 400) {
        ...
        if (index === cards.length - 1) {
            ...
            Animated.timing(this.state.drag, {
                ...
                let update = {};
                if (newTopCard === 'cardA') {
                update = { ...update, cardB: cards[nextCard] };
                }
                if (newTopCard === 'cardB') {
                update = { ...update, cardA: cards[nextCard] };
                }

                this.state.drag.setValue({ x: 0, y: 0 });
                this.state.dragDistance.setValue(0);
                this.setState({ ...update, topCard: newTopCard, sindex: nextCard + 1 });
